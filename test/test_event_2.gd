...
# Stage 2 (effect + chain + choice 통합) 자동 검증.
# 시나리오: 노드 6 (type "repair") 진입 → repair_choice (choice kind) →
#   가지 선택 → 가지의 next (effect kind) → 그 다음 next (dialogue) → chain 종료.
# 두 가지 (팔 수리 / 몸 수리) 모두 검증. once_per 재진입·회귀 검증 포함.
#
# main.tscn 인스턴스 + 시나리오 + 스크린샷 4컷.
# 실행: godot --path "<프로젝트>" res://test/test_event_2.tscn   (스크린샷 위해 --headless 빼기)
...
func _ready() -> void:
    print("=== Stage 2 (effect + chain + choice) 자동 검증 시작 ===")
...
func _run_scenario() -> bool:
    ...
    # ========== 섹션 2: 노드 5 (regression_speech) 통과 ==========
    ...
    ok = _check("event_state.chain_root_id = 'repair_choice'",
        EventManager.event_state.get("chain_root_id") == "repair_choice") and ok

    # ========== 섹션 4: 팔 수리 선택 → chain 진행 (choice → effect → dialogue) ==========
    ...
    ok = _check("chain_root_id 유지 = 'repair_choice'",
        EventManager.event_state.get("chain_root_id") == "repair_choice") and ok
    ok = _check("phase = 'event' 유지 (chain 진행 중)",
        RunManager.run_data.get("phase") == "event") and ok
    ...
    EventManager.advance_line()
    await _shot("2_03_chain_done_back_to_map")
    ...
    # ========== 섹션 7: 회귀 후 once_per 유지 ==========
    RunManager.end_internal_run("cleared")
    await get_tree().process_frame
    await _consume_dialogues()  # 혹시 run_start 발화 있으면 소비 (intro 는 once_per 로 안 발화)

    ok = _check("회귀 후 seen_events[repair_choice] 유지 = 1",
        RunManager.big_run_data["seen_events"].get("repair_choice", 0) == 1) and ok

    RunManager.move_to_node(5)
    RunManager.move_to_node(6)
    ok = _check("회귀 후에도 repair 미발화",
        RunManager.run_data.get("phase") == "map") and ok
    ok = _check("회귀 후에도 event_state 비활성",
        EventManager.event_state.is_empty()) and ok
    ...
    RunManager.move_to_node(5)
    await _consume_dialogues()  # regression_speech
